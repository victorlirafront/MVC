...
test("reservation page shows correct number of available seats", async () => {
  ...
  //we are using await because de information comes from the API
  const seatCountText = await screen.findByText(/10 seats left/i);
  expect(seatCountText).toBeInTheDocument();
});

test("reservation page shows 'sold out' message and NO purchase button if there are no seats availabe", async () => {
  ...
  //we use queryByRole when we expect it to not be in the document
